...
feed, voice, welcome — ships as plugin subprocesses, and no core service
module imports a concrete plugin crate. The alternative was growing typed
domain RPCs in the host (the ADR-0010 pattern the settings panels used), but
that keeps the host permanently domain-aware and turns every query into
protocol surface; the loader goal rules it out. Supersedes the panel-era
framing of ADR-0009 (panels are no longer the only thing pluginized — the

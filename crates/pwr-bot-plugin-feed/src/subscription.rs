...
pub async fn subscribe(
    ...
    let feed = match get_feed_by_source_url(host, url).await? {
        Some(feed) => feed,
        None => {
            let new_feed = create_feed(host, url).await?;
            new_feed
        }
    };

    let feed_id = feed.get("id").and_then(|v| v.as_i64()).unwrap_or(0);
    let feed_name = feed.get("name").and_then(|v| v.as_str()).unwrap_or("Unknown").to_string();

    // Check if already subscribed
    let existing = unsafe {
        host.query_db(
            "SELECT id FROM feed_subscriptions WHERE feed_id = $1 AND subscriber_id = $2",
            &[DbValue::I64(feed_id as i64), DbValue::I64(subscriber_id as i64)],
        )
...
pub async fn unsubscribe(
    ...
    let feed = match get_feed_by_source_url(host, source_url).await? {
        Some(feed) => feed,
        None => return Ok(UnsubscribeResult::NoneSubscribed {
...
pub async fn check_feed_update(
    ...
    let feed_name = feed_row.get("name").and_then(|v| v.as_str()).unwrap_or("").to_string();
    let source_url = feed_row.get("source_url").and_then(|v| v.as_str()).unwrap_or("");
    let items_id = feed_row.get("items_id").and_then(|v| v.as_str()).unwrap_or("");
    let cover_url = feed_row.get("cover_url").and_then(|v| v.as_str()).unwrap_or("").to_string();
    ...
    let platform = platforms.get_platform_by_source_url(source_url)
    ...
    let old_title = match old_item {
        serde_json::Value::Array(arr) => arr.into_iter().next()
            .and_then(|r| r.get("description").and_then(|v| v.as_str()).map(|s| s.to_string())),
        _ => None,
    };

    // Fetch latest from the platform API
    let new_latest = platform.fetch_latest(items_id)
        .await
        .map_err(|e| format!("Platform fetch failed: {e}"))?;

    // Check if version changed
    if let Some(ref old) = old_title {
        if new_latest.title == *old {
            return Ok(FeedUpdateResult::NoUpdate);
        }
    }

...
use crate::service::feed_settings::FeedSettingsService;

/// Service for managing feed subscriptions and updates.
pub struct FeedSubscriptionService {
    pub feed: Box<dyn FeedRepository + Send + Sync>,
    pub feed_item: Box<dyn FeedItemRepository + Send + Sync>,
    pub subscriber: Box<dyn SubscriberRepository + Send + Sync>,
    pub feed_subscription: Box<dyn FeedSubscriptionRepository + Send + Sync>,
    pub platforms: Arc<Platforms>,
    settings: Arc<FeedSettingsService>,
}

impl FeedSubscriptionService {
    /// Creates a new feed subscription service.
    pub fn new(repository: &Repository, platforms: Arc<Platforms>) -> Self {
        Self {
            feed: repository.feed(),
            feed_item: repository.feed_item(),
            subscriber: repository.subscriber(),
            feed_subscription: repository.feed_subscription(),
            platforms,
            settings: Arc::new(FeedSettingsService::new(repository.clone())),
        }
    }
    ...
    pub async fn get_feeds_by_tag(&self, tag: &str) -> Result<Vec<FeedEntity>, ServiceError> {
        Ok(self.feed.select_all_by_tag(tag).await?)
    }
    ...
    pub async fn check_feed_update(
        ...
        let subs = self.feed_subscription.exists_by_feed_id(feed.id).await?;
        ...
        let old_latest: Option<FeedItemEntity> =
            self.feed_item.select_latest_by_feed_id(feed.id).await?;

        let platform = self
            .platforms
            .get_platform_by_source_url(&feed.source_url)
            .ok_or_else(|| ServiceError::UnexpectedResult {
                message: format!("feed platform not found for {}", feed.source_url),
            })?;
        ...
        let new_latest = match platform.fetch_latest(&feed.items_id).await {
            ...
            Err(e) => {
                if matches!(e, FeedError::SourceFinished { .. }) {
                    self.feed.delete(&feed.id).await?;
        ...
        self.feed_item.replace(&new_feed_item).await?;

        Ok(FeedUpdateResult::Updated {
            old_item: old_latest,
            new_item: new_feed_item,
            feed_info: Box::new(platform.get_base().info.clone()),
        })
    }
    ...
    pub async fn get_or_create_feed(&self, source_url: &str) -> Result<FeedEntity, ServiceError> {
            ...
            .await?
        {
            ...
            None => {
                ...
                feed.id = self.feed.insert(&feed).await?;
                ...
                if let Ok(feed_latest) = platform.fetch_latest(&feed.items_id).await {
                    ...
                    self.feed_item.insert(&version).await?;
                }
    ...
    pub async fn get_or_create_subscriber(
            ...
            .await?
        {
            ...
            None => {
                ...
                subscriber.id = self.subscriber.insert(&subscriber).await?;
                subscriber
            }
        };
        Ok(subscriber)
    ...
    /// # Performance
    /// * DB calls: 1
    pub async fn get_feed_settings(&self, guild_id: u64) -> Result<FeedsSettings, ServiceError> {
        self.settings.get(guild_id).await
    }
    ...
    pub async fn update_feed_settings(
        ...
        self.settings.update(guild_id, settings).await
    }
    ...
    async fn create_subscription(
        ...
        self.feed_subscription.insert(&subscription).await?;
        Ok(())
    }
}

...
//! (`FeedBatchModel`) and the exhaustive message vocabulary (`FeedBatchMsg`).
//! The plugin command loop performs the per-URL subscription IO and renders
...
///
/// The batch view has no side effects; the plugin command loop performs the
/// subscription IO and handles the resulting navigation.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum FeedBatchEffect {}

/// The pure update function.
///
/// Each render is a fresh model built by the plugin command loop, so every
/// message is a no-op that returns no effects (expiry included — the batch

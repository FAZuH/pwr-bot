...
//!
//! Initial subscriptions arrive at model construction. Pagination and
//! post-save reloads return [`FeedListEffect`] values; the plugin command loop
//! executes them against the feed service and sends the resulting messages
//! back through the update function.

use std::collections::HashSet;

use pwr_plugin_support::lifecycle::Lifecycle;
...
pub enum FeedListMsg {
    ...
    /// The plugin command loop confirmed a save and is ready to reload.
    Saved,
    /// The plugin command loop loaded a fresh page of subscriptions.
...
///
/// The plugin command loop executes these values against the feed service.
...
/// edited. Pagination and post-save reloads return a [`FeedListEffect`] for
/// the plugin command loop to execute; the returned
/// [`FeedListMsg::SubscriptionsLoaded`] replaces the page in place.

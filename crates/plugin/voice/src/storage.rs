//! PostgreSQL storage and embedded migration execution for the voice plugin.
    ...
    pub async fn migrate(&self) -> anyhow::Result<Vec<String>> {
        ...
        tokio::task::spawn_blocking(move || {
            let mut connection = diesel::PgConnection::establish(&db_url)
                .context("connect to the database for voice migrations")?;
            connection
                ...
                .context("run pending voice migrations")
        })
        .await?
    }
}

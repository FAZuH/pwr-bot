...
pub enum VoiceStatsMsg {
    ...
    SetUser(Option<u64>),
    /// The adapter loaded a fresh stats snapshot at this reference time.
    StatsLoaded {
        data: VoiceStatsData,
        /// Reference time used for the next range and image calculation.
        now: DateTime<Utc>,
    },
...
pub enum VoiceStatsEffect {
    ...
    QueryStats {
        /// Reference time used to build the query range.
        now: DateTime<Utc>,
    ...
    RenderImage {
        /// Reference time used to build the chart range.
        now: DateTime<Utc>,
        time_range: VoiceStatsTimeRange,
        stat_type: GuildStatType,
        is_user: bool,
...
pub fn update(
    ...
    match msg {
        ...
        StatsLoaded { data, now } => {
            model.data = data;
            vec![render_image(model, now)]
        }
        ImageRendered(bytes) => {
...
fn query(model: &VoiceStatsModel, now: DateTime<Utc>) -> VoiceStatsEffect {
...
fn render_image(model: &VoiceStatsModel, now: DateTime<Utc>) -> VoiceStatsEffect {
...
mod tests {
    ...
    fn update(msg: VoiceStatsMsg, model: &mut VoiceStatsModel) -> Vec<VoiceStatsEffect> {
        super::update(msg, model, test_now())
    }
    ...
    fn change_time_range_updates_and_queries() {
        ...
        assert_eq!(
            ...
            vec![VoiceStatsEffect::QueryStats {
                now: test_now(),
                time_range: VoiceStatsTimeRange::Yearly,
    ...
    fn current_streak_counts_consecutive_days() {
        ...
        assert_eq!(model.current_streak(test_now()), 2);
    }

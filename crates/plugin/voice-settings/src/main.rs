...
#[derive(Debug, Clone, PartialEq)]
enum Pending {
    /// The `host.voice.get_settings` issued to load the model before the
    /// first render.
    LoadSettings { invoke_id: u64, guild_id: u64 },
    /// The `host.voice.update_settings` a terminal exit issued before
    ...
    /// The `host.voice.update_settings` an expiry issued: nothing to answer,
...
impl Pending {
    ...
    fn op(&self) -> &'static str {
        match self {
            Pending::LoadSettings { .. } => "host.voice.get_settings",
            Pending::Persist { .. } => "host.voice.update_settings",
            Pending::OpenHub { .. } => "host.open_view",
            Pending::Expire => "host.voice.update_settings",
        }
    }
}

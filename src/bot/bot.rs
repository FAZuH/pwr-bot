...
use crate::{database::database::Database, source::manga_dex_source::MangaDexSource, Config};
...
async fn subscribe(
    ...
    match series_type {
        SeriesType::Manga => {
            if let Some(res) = ctx.data().mangadex_source.get_latest(&series_id).await? {
                ...
                ctx.say(format!( "❌ Manga with series id {} not found on MangaDex", series_id)).await?;
                return Ok(());
            }
        }
        SeriesType::Anime => {
            if let Some(res) = ctx.data().anilist_source.get_latest(&series_id).await? {
                ...
                ctx.say(format!( "❌ Anime with series id {} not found on AniList", series_id)).await?;
                return Ok(());
            }
        }
    }
    ...
    let latest_update_id = ctx.data().db.latest_updates_table.insert(&latest_update).await?;

    let subscriber = SubscribersModel {
        id: 0,
        subscriber_id: {
            if send_into.as_str() == "webhook" { ctx.data().config.webhook_url.clone() } else { user_id }
        },
...
async fn unsubscribe(
    ...
    let latest_update_id = if let Ok(res) = ctx.data().db.latest_updates_table.select_by_model(latest_update_id).await {
        res.id
    } else {
    ...
    if ctx.data().db.subscribers_table.delete_by_model(subscriber).await.is_err() {

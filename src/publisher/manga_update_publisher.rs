...
impl MangaUpdatePublisher {
    pub fn new(
        ...
        info!(
            "Initializing MangaUpdatePublisher with poll interval {:?}",
            poll_interval
        );
        Arc::new(Self {
            db,
    ...
    async fn check_updates(&self) -> anyhow::Result<()> {
        ...
        info!(
            "MangaUpdatePublisher: Found {} manga subscriptions.",
            latest_updates.len()
        );
        ...
        for mut prev_check in latest_updates {
            ...
            debug!(
                "MangaUpdatePublisher: Current latest for series ID {}: chapter {}",
                prev_check.series_id, curr.chapter
            );

            // 4. Compare chapters
            if curr.chapter == prev_check.series_latest {
                debug!(
                    "MangaUpdatePublisher: No new chapter for series ID {}.",
                    prev_check.series_id
                );
                continue;
            }
            info!(
                "MangaUpdatePublisher: New chapter found for series ID {}: {} -> {}. Updating database.",
                prev_check.series_id, prev_check.series_latest, curr.chapter
            );
            ...
            info!(
                "MangaUpdatePublisher: Publishing update event for series ID {}.",
                prev_check.series_id
            );
